...
class Model():
    ...
    def __init__(self, args, data, infer=False):
        ...
        with tf.name_scope('model'):
            ...
            # return zeros tensor [batch_size,state_size]
...
def train(data, model, args):
    with tf.Session() as sess:
        ...
        for i in range(max_iter):
            ...
            train_loss, _, _ = sess.run([model.cost, model.last_state, model.train_op],
                                        feed_dict)
            if i % 10 == 0:
...
def main(infer):
    # define params in traing
    args = HParam()
    # reading data
    data = DataGenerator('./datas/JayLyrics.txt', args)
    # define model
